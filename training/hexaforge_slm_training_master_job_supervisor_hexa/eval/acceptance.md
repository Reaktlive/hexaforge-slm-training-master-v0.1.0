# Acceptance — `hexaforge_slm_training_master_job_supervisor_hexa` SLM

A candidate model is **bind-ready** only when, run at temperature 0 on `seed_data/eval.jsonl`:

1. **Schema-valid** — every output is validated against `contracts/hexaforge_slm_training_master_job_supervisor_hexa/xo.schema.json`
