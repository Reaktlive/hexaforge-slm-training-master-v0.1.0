# SLM model card & training spec — `hexaforge_slm_training_master_data_provenance_recorder_hexa`
...
| Competence | `data_provenance_recorder` |
| Role | `core` |
...
Consume the node's **input** (`contracts/hexaforge_slm_training_master_data_provenance_recorder_hexa/xi.schema.json`, fields: ts, payload) and emit the node's **required output** (`contracts/hexaforge_slm_training_master_data_provenance_recorder_hexa/xo.schema.json`, fields: ts, payload, status). It proposes only — the agent's CCAS gate decides autonomy.
