# SLM model card & training spec — `hexaforge_slm_training_master_eval_gatekeeper_hexa`
...
Consume the node's **input** (`contracts/hexaforge_slm_training_master_eval_gatekeeper_hexa/xi.schema.json`, fields: ts, payload) and emit the node's **required output** (`contracts/hexaforge_slm_training_master_eval_gatekeeper_hexa/xo.schema.json`, fields: ts, payload, status). It proposes only — the agent's CCAS gate decides autonomy.
