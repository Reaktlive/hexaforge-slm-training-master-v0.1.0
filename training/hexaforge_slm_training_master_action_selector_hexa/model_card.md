# SLM model card & training spec — `hexaforge_slm_training_master_action_selector_hexa`
...
Consume the node's **input** (`contracts/hexaforge_slm_training_master_action_selector_hexa/xi.schema.json`, fields: confidence, target_asset, idempotency_key, selected_action, action_parameters, policy_requirements) and emit the node's **required output** (`contracts/hexaforge_slm_training_master_action_selector_hexa/xo.schema.json`, fields: selected_action, target_asset, action_parameters, confidence, policy_requirements, idempotency_key, tenant_id, ts, payload). It proposes only — the agent's CCAS gate decides autonomy.
