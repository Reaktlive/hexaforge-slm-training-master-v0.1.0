"""hexaforge_slm_training_master_intake_octa.port_yo — side effects out (privileged actions, gated by CCAS)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterIntakeOctaYoSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_intake_octa/port-yo")


@router.post("")
async def receive_yo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterIntakeOctaYoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_intake_octa", port="yo", payload=validated.model_dump())
    decision = ccas_decide(action=validated.model_dump(), tier="auto")
    log_event(node_id="hexaforge_slm_training_master_intake_octa", port="yo", payload=validated.model_dump(), ccas_decision=decision)
