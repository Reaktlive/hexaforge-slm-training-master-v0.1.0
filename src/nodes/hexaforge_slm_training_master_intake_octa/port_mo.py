"""hexaforge_slm_training_master_intake_octa.port_mo — macro / cohort signals out (governed by anonymization_contract)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterIntakeOctaMoSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_intake_octa/port-mo")


@router.post("")
async def receive_mo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterIntakeOctaMoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_intake_octa", port="mo", payload=validated.model_dump())
    enforce_contract(payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_intake_octa", port="mo", payload=validated.model_dump())
