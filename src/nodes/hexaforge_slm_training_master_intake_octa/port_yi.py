"""hexaforge_slm_training_master_intake_octa.port_yi — constraints in (policy / config)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterIntakeOctaYiSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_intake_octa/port-yi")


@router.post("")
async def receive_yi(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterIntakeOctaYiSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_intake_octa", port="yi", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_intake_octa", port="yi", payload=validated.model_dump())
