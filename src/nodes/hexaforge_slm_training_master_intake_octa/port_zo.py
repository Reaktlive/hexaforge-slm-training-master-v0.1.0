"""hexaforge_slm_training_master_intake_octa.port_zo — telemetry out"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterIntakeOctaZoSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_intake_octa/port-zo")


@router.post("")
async def receive_zo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterIntakeOctaZoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_intake_octa", port="zo", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_intake_octa", port="zo", payload=validated.model_dump())
