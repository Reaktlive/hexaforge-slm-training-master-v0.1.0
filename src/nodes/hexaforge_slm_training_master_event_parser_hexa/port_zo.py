"""hexaforge_slm_training_master_event_parser_hexa.port_zo — telemetry out"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterEventParserHexaZoSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_event_parser_hexa/port-zo")


@router.post("")
async def receive_zo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterEventParserHexaZoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_event_parser_hexa", port="zo", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_event_parser_hexa", port="zo", payload=validated.model_dump())
