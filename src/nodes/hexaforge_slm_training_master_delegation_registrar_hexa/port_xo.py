"""hexaforge_slm_training_master_delegation_registrar_hexa.port_xo — primary output"""
...
router = APIRouter(prefix="/hexaforge_slm_training_master_delegation_registrar_hexa/port-xo")


@router.post("")
async def receive_xo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterDelegationRegistrarHexaXoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_delegation_registrar_hexa", port="xo", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_delegation_registrar_hexa", port="xo", payload=validated.model_dump())
