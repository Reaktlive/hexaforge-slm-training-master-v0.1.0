"""hexaforge_slm_training_master_decision_hexa.port_xo — primary output"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterDecisionHexaXoSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_decision_hexa/port-xo")


@router.post("")
async def receive_xo(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterDecisionHexaXoSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_decision_hexa", port="xo", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_decision_hexa", port="xo", payload=validated.model_dump())
