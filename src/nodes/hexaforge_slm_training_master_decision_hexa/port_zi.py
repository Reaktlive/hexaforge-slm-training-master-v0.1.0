"""hexaforge_slm_training_master_decision_hexa.port_zi — resources in (knowledge / memory)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterDecisionHexaZiSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_decision_hexa/port-zi")


@router.post("")
async def receive_zi(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterDecisionHexaZiSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_decision_hexa", port="zi", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_decision_hexa", port="zi", payload=validated.model_dump())
