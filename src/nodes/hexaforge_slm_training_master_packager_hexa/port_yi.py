"""hexaforge_slm_training_master_packager_hexa.port_yi — constraints in (policy / config)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterPackagerHexaYiSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_packager_hexa/port-yi")


@router.post("")
async def receive_yi(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterPackagerHexaYiSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_packager_hexa", port="yi", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_packager_hexa", port="yi", payload=validated.model_dump())
