"""hexaforge_slm_training_master_enrichment_hexa.port_zi — resources in (knowledge / memory)"""
from fastapi import APIRouter, HTTPException
from pydantic import ValidationError
from src.shared.contracts import HexaforgeSlmTrainingMasterEnrichmentHexaZiSchema
...
router = APIRouter(prefix="/hexaforge_slm_training_master_enrichment_hexa/port-zi")


@router.post("")
async def receive_zi(payload: dict):
    try:
        validated = HexaforgeSlmTrainingMasterEnrichmentHexaZiSchema(**payload)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=str(e))

    validate_contract(node_id="hexaforge_slm_training_master_enrichment_hexa", port="zi", payload=validated.model_dump())
    log_event(node_id="hexaforge_slm_training_master_enrichment_hexa", port="zi", payload=validated.model_dump())
