...
> **New here?** Start with **[STATUS.md](STATUS.md)** — what is built, what is in progress, and the exact commands to **verify the signed build yourself, offline**. For *how it actually works*, three reads (concept → detail → depth): **[the mental model](docs/MENTAL_MODEL.md)** (the concept in plain terms), **[how it works](docs/HOW_IT_WORKS.md)** (node → port → gate → graph → signature, each pointing to a file), and **[the gate, traced](docs/GATE_TRACE.md)** (one privileged action followed through the code, hop by hop). The UI design brief is in **[`docs/design/`](docs/design/)**. This repo is the editable *handoff*; the signed, offline-verifiable artifact is the **[v0.1.0 release](../../releases)**.
