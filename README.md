...
> **New here?** Start with **[STATUS.md](STATUS.md)** — what is built, what is in progress, and the exact commands to **verify the signed build yourself, offline**. For *how it actually works* — node → port → gate → graph → signature, traced file by file — read **[docs/HOW_IT_WORKS.md](docs/HOW_IT_WORKS.md)**. The UI design brief is in **[`docs/design/`](docs/design/)**. This repo is the editable *handoff*; the signed, offline-verifiable artifact is the **[v0.1.0 release](../../releases)**.
